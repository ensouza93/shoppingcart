...
public class ProdutoEndpoint {
	...
	public Response create(Produto entity) {
		...
		entity.setCategoria(em.find(Categoria.class, entity.getCategoria().getId()));
	...
	public Response findById(@PathParam("id") Long id) {
		TypedQuery<Produto> findByIdQuery = em
				.createQuery(
						"SELECT DISTINCT p FROM Produto p LEFT JOIN FETCH p.categoria WHERE p.id = :entityId ORDER BY p.id",
						Produto.class);
	...
	public List<Produto> listAll(@QueryParam("start") Integer startPosition,
		...
		TypedQuery<Produto> findAllQuery = em
				.createQuery(
						"SELECT DISTINCT p FROM Produto p LEFT JOIN FETCH p.categoria ORDER BY p.id",
						Produto.class);
